...
const container = document.getElementById("bba-three-container");
...
const bpmlabel = createText("#bba-label-BPM");
const zerolabel = createText("#bba-label-zero");
const quadlabel = createText("#bba-label-Quad");
const K0label = createText("#bba-label-K0", "red");
const Kplabel = createText("#bba-label-Kp", "red");
const Kmlabel = createText("#bba-label-Km", "red");
const beamlabel = createText("#bba-label-beam", "red");
...
const bpmReguaX = - 4; // Posicionada à esquerda do quadrupolo, por exemplo, a -4 em X
...
for (let i = 0; i < numMarcadores; i++) {
    ...
    const size = (i === 6) ? sizeMarcadorLarge : sizeMarcadorSmall; // Marcador central (Y=0) é maior
...
function updateFrame(delta, t) {
    ...
    if (step === 0) {
        ...
        beam2.visible = false;
        beam3.visible = false;
        K0label.visible = true;
        Kplabel.visible = false;
        Kmlabel.visible = false;
    }

    const y_input = move * 3 * Math.sin(t * 0.9);
    const dy_prime = -K * y_input;

    for (const [i, beam] of [beam1, beam2, beam3].entries()) {

        let K = move * K_list[i] * 0.7;
        ...
        const distance = screenEndX - quadX;
        positions[6] = screenEndX;
        positions[7] = y_input + (distance * -K * y_input);

        if (beam == beam1) K0label.position.set(positions[6] + 0.5, positions[7], 0);
        if (beam == beam2) Kplabel.position.set(positions[6] + 0.5, positions[7], 0);
        if (beam == beam3) Kmlabel.position.set(positions[6] + 0.5, positions[7], 0);
