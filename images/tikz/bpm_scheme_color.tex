...
\draw[darkgray, densely dashed, line width=0.4pt] (A) -- (C);
\draw[darkgray, densely dashed, line width=0.4pt] (B) -- (D);

\draw[black, thick] (O) circle (\r);

\draw[thick, darkgray, ->] (0,0) -- (\r+\dr, 0) node[right] {$x$};
\draw[thick, darkgray, ->] (0,0) -- (0, \r+\dr) node[above] {$y$};

\draw[fill, blue] ({0.4*\r}, 0) circle (0.1);
\draw[fill, red] ({-0.4*\r}, 0) circle (0.1);
\draw[fill, ForestGreen] (0, {0.4*\r}) circle (0.1);
\draw[fill, BurntOrange] (0, {-0.4*\r}) circle (0.1);
