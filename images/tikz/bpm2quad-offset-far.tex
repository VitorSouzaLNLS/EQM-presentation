...
\begin{tikzpicture}[>=latex]
    ...
    \draw[line width=0.7pt, red] (-3.4, 1.0) -- (0, 0.3) -- (2, -0.6);% node[right, yshift={-0.25em}] {\small{beam}};
    \draw[line width=0.7pt, red] (-3.4, 1.4) -- (0, -0.3) -- (2, -0.8);% node[right, yshift={-0.25em}] {\small{beam}};

    % régua
    \bpmrule{-2}{4}{41}{black}
    \pgfmathsetmacro{\dybba}{-1.17}
    \node[above, black] at (-2, 2) {\small{BPM}};
    \draw[thick, gray, <->] (-2.2, \dybba) -- (-2.2, 0) node[midway, left] {\small $x_\text{\tiny{offset,real}}$};
    \draw[thick, gray, <->] (-3.5, \dybba) -- (-3.5, 0.7) node[midway, left, yshift={-0.5em}] {\small $x_\text{\tiny{offset,BBA}}$};
    \draw[thick, densely dotted, black] (B) -- ++(-1.6, 0);
    \draw[thick, densely dotted, black] (-2, \dybba) -- ++(-1.6, 0);
